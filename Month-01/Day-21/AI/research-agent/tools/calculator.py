...
# Supported mathematical constants and functions
SAFE_NAMES = {
    "pi": math.pi,
    "e": math.e,
    "sqrt": math.sqrt,
...
def _eval_ast_node(node: ast.AST) -> Union[int, float]:
    ...
    elif isinstance(node, ast.BinOp):  # Binary operations (a + b, a * b, a ** b)
        left = _eval_ast_node(node.left)
        right = _eval_ast_node(node.right)
        op_type = type(node.op)
        if op_type in SAFE_OPERATORS:
            # Guard against excessive exponential computations
    ...
    elif isinstance(node, ast.Call):  # Function calls: sqrt(16), log(100)
