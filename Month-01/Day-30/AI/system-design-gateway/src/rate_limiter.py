...
class TokenBucketRateLimiter:
    """
    Multi-tenant Token Bucket Rate Limiter for GenAI Gateways.
    ...
    def consume(self, tenant_id: str, estimated_tokens: int = 100) -> Tuple[bool, str]:
        ...
        if state["token_budget"] < float(estimated_tokens):
            return False, f"TPM quota exceeded for tenant '{tenant_id}' (needed {estimated_tokens}, available {int(state['token_budget'])})"
